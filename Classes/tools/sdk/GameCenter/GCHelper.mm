	...
	[leaderBoard loadScoresWithCompletionHandler:  ^(NSArray *scores, NSError *error)
     {
         if (error != nil){
             ...
             int myLenght = [[NSString stringWithFormat:@"%u",leaderBoard.maxRange] intValue];
             NSLog(@"maxrange下载成功 %u %d",leaderBoard.maxRange, myLenght);
    ...
    if (leaderboardRequest != nil)
    {
        ...
        [leaderboardRequest loadScoresWithCompletionHandler: ^(NSArray *scores, NSError *error) {
            ...
            if ([leaderboardRequest.scores count]>0) {
                ...
                userDefault->setIntegerForKey(RANK_PERCENT, localInfo.rank);
                NSLog(@"rank下载成功 %d,分数%d",localInfo.rank,localInfo.value);
            }
        }];
    }
}
